  ...
  return (
    <div className="min-h-screen bg-ink-50 pt-20 sm:pt-24 pb-16">
      <Container>
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-5 lg:gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="rounded-2xl border border-ink-100 bg-white p-5 shadow-card sm:p-6">
              ...
              <span className="mt-3 inline-block rounded-lg bg-brand-50 px-3 py-1.5 text-lg font-bold text-brand-700">
                Gh&cent;{product.price || 'N/A'}
              </span>
        ...
        <div className="mt-10">
          <SectionHeading title="Related produce" />
          {relatedProducts.length > 0 ? (
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
              {relatedProducts.map((relatedProduct) => (
                <Link
                  to={`/agricdetail/${relatedProduct._id}`}
                ...
                >
                  <ListingCard
                    ...
                    meta={[relatedProduct.region, relatedProduct.town].filter(Boolean).join(", ")}
                  />
                </Link>
              ))}
            </div>
          ) : (
            <EmptyState title="No related produce found" />
          )}
        </div>
