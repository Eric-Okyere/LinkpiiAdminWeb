...
const ShopDetail = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        const relatedResponse = await fetch(`${baseURL}shops/${id}/related`);
  ...
  const handlePostComment = async () => {
    ...
    try {
      const response = await fetch(`${baseURL}shopcomment/${id}/comments`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const saveEditComment = async (commentId) => {
    ...
    try {
      ...
      const response = await fetch(`${baseURL}shopcomment/comments/${commentId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
...
{videos.length > 0 &&
      ...
      {/* Videos */}
        ...
        {/* Product Details */}
        <div className="mt-6 md:mt-0 -ml-10 px-4 sm:px-6 md:px-8 lg:px-12 max-w-full">
  ...
  {relatedProducts.length > 0 ? (
    <div className="flex flex-wrap justify-center gap-6 p-2">
      {relatedProducts.map((relatedProduct) => (
        <Link
          to={`/shopdetail/${relatedProduct._id}`}
