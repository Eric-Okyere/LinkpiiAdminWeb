...
const CarSearch = ({ productFiltered }) => {
          ...
          {productFiltered.map((item) => (
              ...
              <div className="mx-5 bg-gray-200 shadow-lg rounded-lg flex p-1 md:space-x-4 lg:space-x-4">
                ...
                <div className="flex-1">
                  <p className="truncate w-36 font-semibold">{item.name}</p>
                  <p className="truncate w-36 text-sm text-gray-600">{item.region}</p>
                  <p className="truncate w-36 text-sm text-gray-600">{item.town}</p>
                  <p className="truncate w-36 text-sm text-gray-600">{item.location}</p>
                </div>
              </div>
